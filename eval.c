...
*       pole  atom k n;  add a correction to an 1/r^n polar term (adjustable hard shell)
...
*       repclone low high;  find the representative clone in the range
*                            v2 try using the energy rather than rms min
*
...
*      dssp_hbond  constant olabel nlabel clabel hlable
...
*	app atom atom k n;  arbitary pair potential  k/r^n
...
*	range ilow,ihigh ,jlow,jhigh  write out the errors in the current potential for atoms 
...
*       blockopt niter nblock  cngdel, but piecewise superimposed on start
*       mcmin at1 at2 dt number;  typically mcmin ca cb 10 100
*       ldsmin at1 at2 dt number;  typically ldsmin ca cb 10 100
*       ldssearch at1 at2 dt number;  typically ldssearch ca cb 10 100
*       rldsmin at1 at2 dt number;  typically rldsmin ca cb 10 100 <== restricted
*                   rldsmin uses restricted to local dominating set blockopt
*       lds3min at1 at2 at3 dt number;  typically lds3min ca cb c 10 100
*     
...
*          tether all fk   do all of them that are known
*          tether noh fk   do all of them that aren't hydrogen
*         
...
*
*	sadgeom niter ;  simulated annealing  distance geometry
*                        niter > 0 blocked iterations, 
*                        niter < 0 point iterations only
*                        niter == 0 use 100 iterations
*
...
*
*	normal damp cutoff   ;    calculate the normal modes  if damp > 0 output them with frequency < cutoff in wavenumber
*
...
*/ 
int  (*potentials[30])(),(*forces[30])(),nused=(-1);
int eval( ip,op,line )
FILE *ip,*op;
char *line;
{
    ...
    int clone(),statclone(),subclone(),nnclone(),repclone();
    float get_f_variable();
    int v_bond(),f_bond(),v_angle(),f_angle();
	int v_constrain_center(), f_constrain_center();
    int f_tbond(),v_tbond();
    ...
    int v_polar(), f_polar();
    int v_pole(),f_pole(),pole();
    int ttarget(),v_ttarget(),f_ttarget();
    int swarm(),v_swarm(),f_swarm();
    int dssp_hbond(),v_dssp_hbond(),f_dssp_hbond();
    int v_box(),f_box();
    int v_semi(),f_semi();
    int v_damp(),f_damp();
    int v_cube(),f_cube();
    ...
    */ 
    ...
    if( strcmp( &token[0][0], "dssp_hbond" ) == 0 )
    {
	dssp_hbond(ftemp[1],&token[2][0],&token[3][0], &token[4][0],&token[5][0]);
        goto DONE;
    }
    ...
    if( strcmp( &token[0][0], "app") == 0)
    {
	    app( itemp[1],itemp[2], ftemp[3],itemp[4]);
	    goto DONE;
    }
    ...
    if( strcmp (&token[0][0],"pole") == 0 )
    {
	if( itemp[1] >= 0)
	pole( itemp[1],ftemp[2],itemp[3]);
        goto DONE;
    }
    ...
    if( strcmp(&token[0][0],"repclone") == 0)
    {
	if( itemp[1] <= itemp[2])
   	     repclone( op,potentials,nused, itemp[1],itemp[2]);
	else
   	     repclone( op,potentials,nused, itemp[2],itemp[1]);
        goto DONE;
    }
    ...
    {
        if( strcmp( &token[1][0], "noh") == 0 )
        {
            if( no_h_tether( ftemp[2] ) )
            {} else
    ...
    {
        FDnormal( forces,nused,echo,op,ftemp[1],ftemp[2] );
        goto DONE;
    }
    ...
    if( strcmp( &token[0][0], "range" )== 0)
    {
        range_analyze( potentials,nused,itemp[1],itemp[2],itemp[3],itemp[4],op );
        goto DONE;
    }
    ...
    if( strcmp( &token[0][0], "dump" ) == 0 )
    {
        ...
        {if( token[itoken][0] == '\0') goto DONE;
            ...
            if( strcmp(&token[itoken][0],"app") == 0) dump_app( op);
            ...
            if( strcmp(&token[itoken][0],"pdb_tether") == 0) dump_pdb_tether( op,100);
            ...
            if( strcmp(&token[itoken][0],"pole") == 0) dump_pole(op);
            if( strcmp(&token[itoken][0],"dssp_hbond") == 0) dump_dssp_hbond(op);
        }
        goto DONE;
    }
    if( strcmp( &token[0][0], "use" ) == 0 )
    {
        for( itoken=1; itoken<MAXTOKEN; itoken++)
        {
            ...
            if( strcmp(&token[itoken][0],"horestrain") == 0)
            {forces[nused] = f_ho_restrain; potentials[nused++] = v_ho_restrain;}
            ...
            if( strcmp(&token[itoken][0],"semi") == 0)
            {forces[nused] = f_semi; potentials[nused++] = v_semi;}
            if( strcmp(&token[itoken][0],"dssp_hbond") == 0)
            {forces[nused] = f_dssp_hbond; potentials[nused++] = v_dssp_hbond;}
            if( strcmp(&token[itoken][0],"app") == 0)
            {forces[nused] = f_app; potentials[nused++] = v_app;}
            if( strcmp(&token[itoken][0],"tango") == 0)
            {forces[nused] = f_tango; potentials[nused++] = v_tango;}
            if( strcmp(&token[itoken][0],"polar") == 0)
            {forces[nused] = f_polar; potentials[nused++] = v_polar;}
            if( strcmp(&token[itoken][0],"pole") == 0)
            {forces[nused] = f_pole; potentials[nused++] = v_pole;}
            if( strcmp(&token[itoken][0],"constrain") == 0)
            {forces[nused] = f_constrain_center; potentials[nused++] = v_constrain_center;}
        }
        goto DONE;
    }
    ...
    if( strcmp( &token[0][0], "sadgeom" ) == 0)
    {
        if( nused <= 0) goto DONE;
        sa_dgeom(echo,op, potentials,nused,itemp[1],itemp[2],ftemp[3]);
        goto DONE;
    }
    ...
    if( strcmp( &token[0][0], "mcmin" ) == 0 )
	{
        if( nused <= 0) goto DONE;	
	mcmin(op,potentials,forces,nused, &token[1][0],&token[2][0],ftemp[3],itemp[4]);
	goto DONE;
	}
    if( strcmp( &token[0][0], "ldsmin" ) == 0 )
	{
        if( nused <= 0) goto DONE;	
	ldsmin(op,potentials,forces,nused, &token[1][0],&token[2][0],ftemp[3],itemp[4]);
	goto DONE;
	}
    if( strcmp( &token[0][0], "ldssearch" ) == 0 )
	{
        if( nused <= 0) goto DONE;	
	ldssearch(op,potentials,forces,nused, &token[1][0],&token[2][0],ftemp[3],itemp[4]);
	goto DONE;
	}
    if( strcmp( &token[0][0], "lds3min" ) == 0 )
	{
        if( nused <= 0) goto DONE;	
	lds3min(op,potentials,forces,nused, &token[1][0],&token[2][0],&token[3][0],ftemp[4],itemp[5]);
	goto DONE;
	}
    if( strcmp( &token[0][0], "rldsmin" ) == 0 )
	{
        if( nused <= 0) goto DONE;	
	restricted_ldsmin(op,potentials,forces,nused, &token[1][0],&token[2][0],ftemp[3],itemp[4]);
	goto DONE;
	}
    if( strcmp( &token[0][0], "blockopt" ) == 0 )
	{
        if( nused <= 0) goto DONE;	
	if( itemp[2] > a_number()/3) itemp[2] = a_number()/3;
	if( itemp[2] < 1) itemp[2] = a_number()/9;
	if( itemp[2] < 1) itemp[2] = a_number()/3;
	block_optimizer(potentials,forces,nused, itemp[1],itemp[2], echo);
        goto DONE;
	}
    if( strcmp( &token[0][0], "pccngdel" ) == 0 )
	{
        if( nused <= 0) goto DONE;
        pc_cngdel( potentials,forces,nused,itemp[1],itemp[2],ftemp[3],echo);
        goto DONE;
	}
    ...
    if( strcmp( &token[0][0], "flipper" ) == 0 )
    {
        if( nused <= 0) goto DONE;
        flipper( op, potentials,forces,nused,&token[1][0],&token[2][0],ftemp[3],itemp[4]);
        goto DONE;
    }
    /*      polytope imin,imax,niter,vstart,vfinal  polytope of a range
    */
...
//	void buster(int (**)(),int, int,FILE *);
    ...
    if( strcmp( &token[0][0], "tango" ) == 0)
    {
        tango( itemp[1],itemp[2],itemp[3],itemp[4],
            ftemp[5],
            ftemp[6],
            ftemp[7],
            ftemp[8],
    ...
    if( strcmp( &token[0][0], "dance") == 0)
    {
         tango_search( op, potentials,nused,itemp[1]);
	goto DONE;
    }
#ifdef TIME 
