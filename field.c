...
//    fprintf(where,"\n       2 !NTITLE\nREMARKS AMMP xplor map                                                                                 \nREMARKS                                                                                                 \n");
    fprintf(where,"%s\n",line);
// number of lines of comments a80 style (AAAAARGH!)
    sprintf(line,"%8i",2);
    for( i=0; i< 80; i++) if( line[i] < ' ' ) line[i] = ' ' ;
    fprintf(where,"%s\n",line);
    for( i=0; i< 80; i++) line[i] = ' '; line[80] = '\0';
    sprintf(line,"%s","REMARKS  AMMP xplor map");
    for( i=0; i< 80; i++) if( line[i] < ' ' ) line[i] = ' ' ;
    fprintf(where,"%s\n",line);
    for( i=0; i< 80; i++) line[i] = ' '; line[80] = '\0';
    sprintf(line,"%s","REMARKS");
    for( i=0; i< 80; i++) if( line[i] < ' ' ) line[i] = ' ' ;
    fprintf(where,"%s\n",line);
    fprintf(where,"%8i%8i%8i%8i%8i%8i%8i%8i%8i\n",
//            ny,jmin,jmax,nx,imin,imax,nz,kmin,kmax);
            nx,imin,imax,ny,jmin,jmax,nz,kmin,kmax);
// official is ZXY
// map_man likes ZYX  so does pymol
//    fprintf(where,"%12.5e%12.5e%12.5e%12.5e%12.5e%12.5e\nZXY",
    fprintf(where,"%12.5e%12.5e%12.5e%12.5e%12.5e%12.5e\nZYX",
            xmax-xmin,ymax-ymin,zmax-zmin,90.,90.,90.);
    /* now do the work we loop with z outer then y then x
